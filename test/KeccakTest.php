...
class KeccakTest extends PHPUnit\Framework\TestCase
{
    ...
    public function testHash()
    {
        ...
        foreach($v as $bitsize => $vectors){
            foreach($vectors as $testcase){
                $this->assertEquals(Keccak::hash($testcase[0], $bitsize), $testcase[1]);

                $this->assertEquals(Keccak::hash($testcase[0], $bitsize, true), hex2bin($testcase[1]));
    ...
    public function testShake()
    {
       $v = [
            128 => [
                ...
                [256, hex2bin(self::SHORT), '3a0faca70c9d2b81d1064d429ea3b05ad27366f64985379ddd75bc73d6a83810'],
                [256, hex2bin(self::LONG), '14236e75b9784df4f57935f945356cbe383fe513ed30286f91060759bcb0ef4b'],
        ...
        foreach($v as $bitsize => $vectors){
            foreach($vectors as $testcase){
                $this->assertEquals(Keccak::shake($testcase[1], $bitsize, $testcase[0]), $testcase[2]);

                $this->assertEquals(Keccak::shake($testcase[1], $bitsize, $testcase[0], true), hex2bin($testcase[2]));
    ...
    public function testSelfTestException()
    {
        ...
        try  {
            ...
            $this->assertEquals($e->getMessage(), 'Keccak self test failed!');
    ...
    public function testSelfTestPreviousFailure()
    {
        ...
        $this->expectExceptionMessage('Keccak previous self test failed!');
        Keccak::hash('',224);
    }
